...
async def ingest_bulk_folders(
    ...
    try:
        ...
        for folder_path in folder_paths:
            ...
            logger.info(
                f"Found {len(files_to_process)} supported files in {folder_path}")
            ...
            for i in range(0, len(files_to_process), chunk_size):
                ...
                if valid_results:
                    db.insert_documents(
                        valid_results, user_id=current_user["id"])
...
async def update_document(
    ...
    try:
        ...
        success = db.update_document(
            doc_id, new_simbadoc, user_id=current_user["id"])
        if not success:
            raise HTTPException(
...
async def preview_document(
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Error encoding filename '{safe_filename}': {str(e)}")
